...
def api_data():
    ...
    # Granted this month
    c.execute(
        "SELECT COUNT(*) FROM entry_log WHERE timestamp LIKE ? AND access_granted=1",
        (month_prefix,)
    )
    ...
    # Denied this month
    c.execute(
        "SELECT COUNT(*) FROM entry_log WHERE timestamp LIKE ? AND access_granted=0",
        (month_prefix,)
    )
    ...
    try:
        ...
        cloud_reduction = round(
            100 - (ocr_cloud / max(total_alltime, 1) * 100), 1
        )
    ...
    # Residents
    c.execute(
        "SELECT COUNT(*) FROM vehicle_profile WHERE classification='RESIDENT'"
    )
    ...
    # Decision counts
    c.execute("""
        SELECT access_granted, flag, COUNT(*)
        FROM entry_log
        WHERE timestamp LIKE ?
        GROUP BY access_granted, flag
    """, (month_prefix,))
    ...
    # Blacklist table
    c.execute("""
        SELECT plate_id, reason, added_at
        FROM blacklist
    ...
    blacklist = [{
        'plate': r[0],
        'reason': r[1],
        'added': r[2][:10] if r[2] else ''
    } for r in c.fetchall()]

    # Vehicle profiles
    c.execute("""
        SELECT p.plate_id, p.visit_count, p.avg_entry_hour,
               p.classification, p.last_seen,

               COALESCE(
                   (SELECT risk_score
                    FROM entry_log
                    WHERE plate_id=p.plate_id
                    ORDER BY id DESC
                    LIMIT 1),0)

        FROM vehicle_profile p
    ...
    profiles = [{
        ...
        'avg_hour': r[2] or 0,
        'classification': r[3],
        'last_seen': r[4][:10] if r[4] else 'Never',
        'risk': r[5] or 0.0
    } for r in c.fetchall()]
